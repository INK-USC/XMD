<template>
  <el-dialog
    :title="'Explanation Generation'"
    v-model="dialogVisible"
    v-on:update:visible="$emit('update:dialogVisible', $event)"
    @close="$emit('update:dialogVisible', false)"
    width="40%"
  >
    <div class="container">
      <div class="row">
          <br><br>
            <p class="text" style="text-align: justify">
              After you input the tran data, you need to load the model trained on the train data. <br>
              Use that model (from local or huggingface) to generate explanations for the train data. <br>
            </p>
      </div>
    </div>

    <template #footer>
      <span class="dialog-footer">
        <el-button @click="() => $emit('update:dialogVisible', false)">
          Understand
        </el-button>
      </span>
    </template>
  </el-dialog>
</template>

<script>

// the form when user create/edit a project
export default {
  name: "GenerateExplanationTutorial",
  props: { dialogVisible: Boolean, existingInfo: Object },
  setup() {
  },
  data() {
    return {};
  },
  methods: {
  },
};
</script>

<style scoped></style>
